...
void randomDelay(void) {
  ...
  /* type-casting the 16-bit TCNT1 as an 8-bit number keeps
     ...
     // below it takes the random value and go backward until it reaches 0. so if the value of the first 8 bits is 128 it will do 128,127,126 .. with 10ms in between
     // so max value is 255 x 10ms = 2.5 seconds max. plus the 1 s delay. so the delay is between 1 and 3.5s
     // when we write uint8_t randomTime we take the 8 leats significant bits. so 27098 will be 128. when casted away. 
  while (--randomTime) {
